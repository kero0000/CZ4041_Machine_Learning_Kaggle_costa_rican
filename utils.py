...
def generate_sumbit_csv(model,text=""):
    ...
    submit_df.to_csv('data/test_submit_{}_{}.csv'.format(text,datetime.datetime.now().strftime("%d-%m-%Y_%H_%M_%S_%f")))
    print('save to '+'data/test_submit_{}_{}.csv'.format(text,datetime.datetime.now().strftime("%d-%m-%Y_%H_%M_%S_%f")))
    print('distribution:')
    for i in range(1,5):
